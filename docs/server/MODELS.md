...
### 3.1 System Config Model (`SystemConfig.ts`)
...
## 3.2 Department Config Model (`DepartmentConfig.ts`) [NEW]
...
### 3.3 Recruitment Generation Model (`RecruitmentGeneration.ts`)
...
### 4.1 Master Interview Slot (`MasterInterviewSlot.ts`) [NEW]
...
### 4.2 Interviewer Availability (`InterviewerAvailability.ts`) [NEW]
...
### 5.1 Candidate Model (`Candidate.ts`)
...
### 6.1 Audit Log Model (`AuditLog.ts`)
