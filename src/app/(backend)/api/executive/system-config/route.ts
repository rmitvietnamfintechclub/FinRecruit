...
import { withActiveRBAC } from '@/app/(backend)/guards/auth&RBAC';
    ...
    return NextResponse.json(
      {
        success: false,
        message:
          e instanceof Error ? e.message : 'Failed to load system config.',
      },
      { status: 500 }
    );
  }
...
export const POST = withActiveRBAC(
  'Executive Board',
...
export const PATCH = withActiveRBAC(
  'Executive Board',
  async (req: NextRequest, { session }) => {
